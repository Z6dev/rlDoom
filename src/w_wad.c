...
void W_AddFile(d_char *filename) {
    ...
    if (!D_StrCaseEqual(GetFileExtension(filename) + 1, "wad")) {
        ...
        // WAD file
        header.identification[0] = file_data[0];
        file_data++;
        header.identification[1] = file_data[0];
        file_data++;
        header.identification[2] = file_data[0];
        file_data++;
        header.identification[3] = file_data[0];
        file_data++;
        header.numlumps = LONG(*((d_int *)file_data));
        file_data += sizeof(d_int);
        header.infotableofs = LONG(*((d_int *)file_data));
        file_data += sizeof(d_int);
...
//
void *
W_CacheLumpNum(d_int lump, d_int tag) {
...
//
void *
W_CacheLumpName(d_char *name, d_int tag) {
